...
test('reader selection survives source and article filter changes', async () => {
  ...
  try {
    ...
    // SS-4：Quick Switcher 的“管理来源”进入独立 Source Manager；管理页有完整来源操作，但 Reader 保持当前文章。
    await page.locator('.source-switcher-trigger').click()
    await sourcePicker.locator('.source-switcher-manage').click()
    const sourceManager = page.locator('.source-manager-overlay')
    await expect(sourcePicker).toHaveCount(0)
    await expect(page.locator('.app-shell')).toHaveAttribute('data-source-manager-open', 'true')
    await expect(sourceManager).toBeVisible()
    await expect(sourceManager.locator('.source-pane.embedded-source-pane')).toBeVisible()
    const managedFeed = sourceManager.locator('.source-item').filter({ hasText: 'OrigRead E2E Feed' })
    await expect(managedFeed).toBeVisible()
    await expect(managedFeed.locator('.source-refresh-button')).toBeVisible()
    await expect(managedFeed.locator('.source-settings-button')).toBeVisible()
    await expect(sourceManager.locator('.source-manager-close')).toBeFocused()
    await expect(page.locator('.article-heading h1')).toContainText('OrigRead E2E Article 1')
    await page.keyboard.press('Escape')
    await expect(sourceManager).toHaveCount(0)
    await expect(page.locator('.app-shell')).toHaveAttribute('data-source-manager-open', 'false')
