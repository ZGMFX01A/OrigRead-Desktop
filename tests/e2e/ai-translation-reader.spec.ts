...
test('reader generates AI summary and full-article translation through main-process providers', async () => {
  ...
  try {
    ...
    const leftResizeHandle = dockedSummary.getByRole('separator',{name:'拖动调整 AI 面板宽度'})
    const [leftHandleBox, leftPanelBox] = await Promise.all([leftResizeHandle.boundingBox(), dockedSummary.boundingBox()])
    if (!leftHandleBox || !leftPanelBox) throw new Error('Left AI panel resize geometry is unavailable')
    const leftStartX = leftHandleBox.x + leftHandleBox.width / 2
    const leftY = leftHandleBox.y + Math.min(24, leftHandleBox.height / 2)
    await page.mouse.move(leftStartX, leftY)
    await page.mouse.down()
    await page.mouse.move(leftStartX + 400 - leftPanelBox.width, leftY, {steps:4})
    await expect.poll(async()=>Math.round((await dockedSummary.boundingBox())?.width??0)).toBe(400)
    await page.mouse.up()
    await expect.poll(async () => page.evaluate(async () => (await window.origread.getSettings()).aiSummaryPanelSize)).toBe(400)
    ...
    const resizeHandle = dockedSummary.getByRole('separator',{name:'拖动调整 AI 面板宽度'})
    const [handleBox, panelBox] = await Promise.all([resizeHandle.boundingBox(), dockedSummary.boundingBox()])
    if (!handleBox || !panelBox) throw new Error('AI panel resize geometry is unavailable')
    const startX = handleBox.x + handleBox.width / 2
    const y = handleBox.y + Math.min(24, handleBox.height / 2)
    await page.mouse.move(startX, y)
    await page.mouse.down()
    await page.mouse.move(startX + panelBox.width - 420, y, {steps:4})
    await expect.poll(async()=>Math.round((await dockedSummary.boundingBox())?.width??0)).toBe(420)
    await page.mouse.up()
