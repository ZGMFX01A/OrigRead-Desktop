...
test('a 470-item RSS remains fully visible in feed scope even though the global window is 200', async () => {
  ...
  try {
    ...
    await expect(page.locator('.article-destination-item').filter({ hasText: '全部文章' })).toContainText('470')
    await expect(page.locator('.article-destination-item').filter({ hasText: '未读' })).toContainText('470')
