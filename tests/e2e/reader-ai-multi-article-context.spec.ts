...
test('Reader AI persists up to five extra articles and freezes each request context', async () => {
  ...
  try {
    ...
    await expect(page.locator('.reader-ai-article-picker-error')).toHaveCount(0)
    await expect(picker.locator('.reader-ai-article-picker-list > button').first()).toContainText('Attached Context Article')

    // Reader AI floating surfaces are mutually exclusive. Opening another popup or clicking a
    // normal composer action must close the previous popup instead of stacking layers.
    await page.locator('.reader-ai-composer-add').click()
    await expect(picker).toBeHidden()
    await expect(page.locator('.reader-ai-composer-actions-popover')).toBeVisible()
    await page.locator('.reader-ai-model-label').click()
    await expect(page.locator('.reader-ai-composer-actions-popover')).toBeHidden()
    await expect(page.locator('.reader-ai-model-popover')).toBeVisible()
    const forceSearch = page.locator('.reader-ai-web-search-toggle')
