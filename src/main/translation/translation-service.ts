...
export class TranslationService{
  ...
  constructor(private readonly library:LibraryRepository,private readonly reader:ReaderContentService,private readonly settings:TranslationSettingsRepository,private readonly aiSettings:AiSettingsRepository,private readonly cacheDir:string,private readonly aiProvider=new OpenAiCompatibleProvider(),private readonly promptCustomizer?:LlmTaskPromptCustomizer){}
  ...
  async translateArticle(articleId:string,target?:TranslationTarget,forceRefresh=false,signal?:AbortSignal):Promise<TranslationDocument>{
    ...
    const baseAiSystemPrompt=actualTarget.type==='ai'?buildAiTranslationSystemPrompt(settings.targetLanguage):null
    const promptCustomization=baseAiSystemPrompt?(this.promptCustomizer?.customize('TRANSLATION',baseAiSystemPrompt)??{systemPrompt:baseAiSystemPrompt,skillId:null,cacheVariant:''}):null
    const cacheFile=this.cacheFile(articleId,article.title,source.html,actualTarget,settings.targetLanguage,settings.displayMode,promptCustomization?.cacheVariant??'')
    ...
    const result=actualTarget.type==='traditional'?await this.translateTraditional(actualTarget.provider,texts,settings.targetLanguage,signal):await this.translateAi(article.title,actualTarget,texts,settings.targetLanguage,promptCustomization!.systemPrompt,signal)
  ...
  private async translateTraditional(type:TranslationProviderType,texts:string[],targetLanguage:string,signal?:AbortSignal):Promise<TranslationBatchResult>{const provider=this.providers[type];const config=this.settings.current().providers.find((item)=>item.type===type);if(!config)throw new Error('翻译服务配置不存在');const segments=splitAll(texts,provider.maxSegmentCharacters);const translated:string[]=[];let detected:string|null=null;let index=0;while(index<segments.length){signal?.throwIfAborted();const batch:Segment[]=[];let chars=0;while(index<segments.length&&batch.length<provider.maxBatchItems){const segment=segments[index]!;if(batch.length&&chars+segment.text.length>provider.maxBatchCharacters)break;batch.push(segment);chars+=segment.text.length;index++}const result=await provider.translate(batch.map((item)=>item.text),detected,targetLanguage,{endpoint:config.endpoint,region:config.region,apiKey:this.settings.getApiKey(type)});signal?.throwIfAborted();detected=detected??result.detectedSourceLanguage;translated.push(...result.texts)}return{texts:mergeSegments(segments,translated,texts.length),detectedSourceLanguage:detected}}
  private async translateAi(articleTitle:string,target:Extract<TranslationTarget,{type:'ai'}>,texts:string[],targetLanguage:string,systemPrompt:string,signal?:AbortSignal):Promise<TranslationBatchResult>{const ai=this.aiSettings.current();const profile=ai.providers.find((item)=>item.id===target.providerId);if(!profile)throw new Error('AI 服务不存在');const capability=resolveAiProviderCapability(profile,target.model);const segments=splitAll(texts,3500);const translated:string[]=[];const context:Array<[string,string]>=[];let index=0;while(index<segments.length){signal?.throwIfAborted();const batch:Segment[]=[];let chars=0;while(index<segments.length&&batch.length<24){const segment=segments[index]!;if(batch.length&&chars+segment.text.length>8000)break;batch.push(segment);chars+=segment.text.length;index++}const completed=await this.aiProvider.completeDetailed(systemPrompt,buildAiTranslationUserPrompt(articleTitle,batch.map((item)=>item.text),context),{endpoint:profile.endpoint,model:target.model,apiKey:this.aiSettings.getApiKey(profile.id),outputTokenLimitStyle:capability.outputTokenLimitStyle,strictStreamTermination:capability.strictStreamTermination},signal);const output=parseAiTranslationResponse(completed.content,batch.length);translated.push(...output);batch.forEach((segment,i)=>{context.push([segment.text,output[i]!]);while(context.length>4||context.reduce((sum,item)=>sum+item[0].length+item[1].length,0)>2000)context.shift()})}return{texts:mergeSegments(segments,translated,texts.length),detectedSourceLanguage:null}}
  private cacheFile(articleId:string,title:string,content:string,target:TranslationTarget,language:string,mode:string,customizationVariant:string):string{const key=createHash('sha256').update(JSON.stringify({v:2,articleId,title,content,target,language,mode,customizationVariant})).digest('hex');return join(this.cacheDir,`${key}.json`)}
}
