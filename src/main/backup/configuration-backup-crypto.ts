...
export function decryptConfigurationSecrets(value:EncryptedBackupSecrets,password:string):ConfigurationBackupSecrets{
  ...
  try{const decipher=createDecipheriv('aes-256-gcm',key,iv);decipher.setAAD(AAD);decipher.setAuthTag(tag);const plain=Buffer.concat([decipher.update(ciphertext),decipher.final()]).toString('utf8');const parsed=JSON.parse(plain) as ConfigurationBackupSecrets;return{translationApiKeys:parsed.translationApiKeys??{},aiApiKeys:parsed.aiApiKeys??{},...(parsed.webSearchApiKeys===undefined?{}:{webSearchApiKeys:parsed.webSearchApiKeys})}}catch{throw new Error('备份密码错误或加密凭据已损坏')}
}
